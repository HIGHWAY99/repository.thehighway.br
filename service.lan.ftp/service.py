...
# ===================================================================
try: LaniP=str(socket1.gethostbyname_ex('')[2]).replace("'","").replace('[','').replace(']','')
except: LaniP='localhost'; print 'local lan ip:  error on attempting to catch ip'; 
#print LaniP
# ===================================================================
...
while (not xbmc.abortRequested): #while (ENABLED==True) and (not xbmc.abortRequested):
	...
	if (ENABLED==False) and (ClosedYetError==False) and (ClosedYet==False) and (STARTUP==False):
		...
		try: server.serve_forever(timeout=10,blocking=False)
		except: pass
		#xbmc.sleep(1000)
		time1.sleep(1)
