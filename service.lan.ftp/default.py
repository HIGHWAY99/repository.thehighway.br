...
try: LiP=str(socket.gethostbyname_ex('')[2]).replace("'","").replace('[','').replace(']','')
except: LiP='localhost'; deb('local lan ip','error on attempting to catch ip'); 
...
def ShowIP(): 
	if tfalse(addst("show-netsettings","false"))==True: 
		addon.add_directory({'mode':'ShowIP'},{'title':cFL('Local IP[CR]@ Network Settings','darkorange')},is_folder=True,fanart=_artFanart,img=artp("LocalIP"))
	if tfalse(addst("show-ftpurl","false"))==True: 
		addon.add_directory({'mode':'System.ExecWait','url':'ftp://'+LiP+':'+addst('port','2121')+'/'},{'title':'ftp://'+cFL(LiP,'darkorange')+':'+cFL(addst('port','2121'),'purple')+'/[CR]'+'ftp://'+cFL('[username]','white')+':'+cFL('[password]','white')+'@'+cFL(LiP,'darkorange')+':'+cFL(addst('port','2121'),'purple')+'/'},is_folder=True,fanart=_artFanart,img=artp("FTPUrlExample"))
	#addon.add_directory({'mode':'Refresh'},{'title':'ftp://'+cFL(LiP,'darkorange')+':'+cFL(addst('port','2121'),'purple')+'/'},is_folder=True,fanart=_artFanart,img=_artIcon)
	#addon.add_directory({'mode':'Refresh'},{'title':'ftp://'+cFL('[username]','white')+':'+cFL('[password]','white')+'@'+cFL(LiP,'darkorange')+':'+cFL(addst('port','2121'),'purple')+'/'},is_folder=True,fanart=_artFanart,img=_artIcon)
...
def GoToFTP(tn):
	try:
		url='ftp://'+addst(tn+"-user","")+':'+addst(tn+"-pass","xbmchub")+'@'+(LiP)+':'+addst('port','2121')+'/'
