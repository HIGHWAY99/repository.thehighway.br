...
def nURL(url,method='get',form_data={},headers={},html='',proxy='',User_Agent='',cookie_file='',load_cookie=False,save_cookie=False):
	...
	if   method.lower()=='get':
		...
		try: 
			...
			if 'streamlive.to/view/' in url:
				try: html=net.http_GET(url.replace('streamlive.to/view/','ilive.to/view/'),headers=headers).content
				except: html=dhtml
			elif 'ilive.to/api/' in url:
				try: html=net.http_GET(url.replace('ilive.to/api/','streamlive.to/api/'),headers=headers).content
				except: html=dhtml
			elif 'streamlive.to/api/' in url:
				try: html=net.http_GET(url.replace('streamlive.to/api/','ilive.to/api/'),headers=headers).content
				except: html=dhtml
			else: html=dhtml
		##except: html=dhtml
...
def PlayItCustom(url,stream_url,img,title,studio=''):
	...
	infoLabels={'Title':title,'Genre':'Live','Studio':studio,"ShowTitle":title,"Plot":url+'\n'+stream_url}
	...
	if tfalse(addst("playback_urlonly"))==True:
		try: play=xbmc.Player(PlayerMeth); play.play(stream_url)
		except: pass
	else:
		try: play=xbmc.Player(PlayerMeth); play.play(PL)
		except:
